...
SIMD_Scores <- readRDS("/conf/linkage/output/lookups/Unicode/Deprivation/postcode_2026_1_simd2020v2.rds") %>%
  select(FormattedPostcode = pc8, simd_quintile = simd2020v2_sc_quintile, simd_decile = simd2020v2_sc_decile)
...
FullDatabase <- scidc_cohort %>% 
  ...
  left_join(SIMD_Scores, by = 'FormattedPostcode')
...
OutputData <- FullDatabase %>% 
  ...
  select(id, chi_formatted, FormattedPostcode, StartKRT, First_Type, First_Unit, PRD_Code, Sex = Gender,
         ...
         date_first_referral, KRTChangeDate = date1, RRT.Modality, Last_Unit, simd_quintile, simd_decile)


#missing: dt_tx_status
...
saveWorkbook(
  ...
  file = paste0(OutputFolder,'/ThureaduEDRIS-2425-0260_OutputDataV3.xlsx'),
  overwrite = TRUE
)
